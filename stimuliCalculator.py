...
def readFile(filename):
	...
	personName = filename[:fo.name.find(".txt")]
	result = fo.read()
	resultArray = result.splitlines()
	resultFile = open("%s_score_comparison.txt" % (personName),"w")
...
def calcOrgSize(org, donation, person):
	...
	cur.execute(query, (person, donation))
	ans= cur.fetchall()
	return ans[0][0]
	...
	while(status):
		...
		if fileName == '#':
			...
			try:
				...
				print("%s does not exist in the 'textFiles' directory" % fileName)
